...
def define_loss(network, params):
    ...
    sindy_coefficients = params['coefficient_mask']*network['sindy_coefficients']*1e-2

    losses = {}
    losses['rekon'] = tf.reduce_mean(tf.square(x - x_decode))
    if params['model_order'] == 1:
        losses['sindy_z'] = tf.reduce_mean(tf.square(dz - dz_predict))
        losses['sindy_x'] = tf.reduce_mean(tf.square(dx - dx_decode))
    else:
        losses['sindy_z'] = tf.reduce_mean(tf.square(ddz - ddz_predict))
        losses['sindy_x'] = tf.reduce_mean(tf.square(ddx - ddx_decode))
    losses['cons'] = tf.reduce_mean(tf.abs(sindy_coefficients))

    loss_z1 = tf.reduce_mean(tf.square(z[:, 0] - x[:, 0]) )
    losses['z1'] = loss_z1

    loss = params['loss_weight_rekon'] * losses['rekon'] \
           + params['loss_weight_sindy_z'] * losses['sindy_z'] \
           + params['loss_weight_sindy_x'] * losses['sindy_x'] \
           + params['loss_weight_z1'] * losses['z1'] \
           + params['loss_weight_cons'] * losses['cons']

    loss_refinement = params['loss_weight_rekon'] * losses['rekon'] \
                      + params['loss_weight_sindy_z'] * losses['sindy_z'] \
                      + params['loss_weight_sindy_x'] * losses['sindy_x'] \
                      + params['loss_weight_z1'] * losses['z1'] \
                      + params['loss_weight_cons'] * losses['cons']
