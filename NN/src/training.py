...
def train_network(training_data, val_data, params):
    ...
    with tf.Session() as sess:
        ...
        final_losses = sess.run((losses['rekon'], losses['sindy_x'], losses['sindy_z'],
                                 losses['z1'] , losses['cons']),
        ...
        results_dict['loss_rekon'] = final_losses[0]
        results_dict['loss_decoder_sindy'] = final_losses[1]
        results_dict['loss_sindy'] = final_losses[2]
        results_dict['loss_z1'] = final_losses[3]
        results_dict['loss_cons'] = final_losses[4]
...
def print_progress(sess, i, loss, losses, train_dict, validation_dict, x_norm, sindy_predict_norm):
    ...
    decoder_losses = sess.run((losses['rekon'], losses['sindy_x']), feed_dict=validation_dict)
    loss_ratios = (decoder_losses[0]/x_norm, decoder_losses[1]/sindy_predict_norm)
    print("rekon loss ratio: %f, rekon SINDy loss  ratio: %f" % loss_ratios)
